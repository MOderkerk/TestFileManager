...
public class FileUploadIntegrationTests {
	...
	public void shouldUploadFile() throws Exception {
		...
		ResponseEntity<String> response = this.restTemplate.postForEntity("/?stage=aete&mandant=M00", map,
				String.class);

		assertThat(response.getStatusCode()).isEqualByComparingTo(HttpStatus.FOUND);
		assertThat(response.getHeaders().getLocation().toString()).startsWith("http://localhost:" + this.port + "/");
		then(storageService).should().store(any(MultipartFile.class), any(String.class), any(String.class));
	}
	...
	public void shouldDownloadFile() throws Exception {
		...
		ResponseEntity<String> response = this.restTemplate.withBasicAuth(user, password)
